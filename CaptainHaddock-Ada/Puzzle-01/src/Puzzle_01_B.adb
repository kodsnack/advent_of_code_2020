...
begin
    ...
    while Has_Element(index_A) loop
        ...
        while  Has_Element(index_B) loop
            index_C:= Next(index_B);
            while  Has_Element(index_C) loop
                -- put(Standard_Error,"."); -- breadcum ...
                if Element(index_A) + Element(index_B) + Element(index_C) = Special_expense then
                    -- Found Special_expense !
                    new_line;
                    put(Expense'Image(Special_expense) &" = ");
                    put(Expense'Image(Element(index_A)) &" + ");
                    put(Expense'Image(Element(index_B)) &" + ");
                    put(Expense'Image(Element(index_C)));
                    new_line;
                    -- Print-out the magic Number !
                    put("Puzzle #1 Number = ");
                    put(Expense'Image(Element(index_A) * Element(index_B) * Element(index_C)));
                    new_line;
                end if;
                exit when Element(index_A)+Element(index_B)+Element(index_C) > Special_expense;
                Next(index_C);
            end loop;
            Next(index_B);
        end loop;
