...
constexpr auto reverseimpl = [](uint32_t n, const uint32_t N = 10) {
    uint32_t ret = 0;
...
constexpr std::array<uint32_t, 1024> table = []() {
    std::array<uint32_t, 1024> ret = {0,};
    for (size_t i = 0; i < 1024; i++) ret[i] = reverseimpl(i);
    return ret;
}();

auto reverse = [](uint32_t n) { return table[n]; };
auto rot90 = [](const piece &p) {
    return piece{p.id, p.ri, reverse(p.up), p.le, reverse(p.dn), p.flip,p.rots + 1};
};
auto flip = [](const piece &p) {
    return piece{p.id, p.dn, reverse(p.le), p.up, reverse(p.ri), !p.flip, p.rots};
};


bool solve(const std::vector<piece> &tiles, std::vector<piece> &grid, std::vector<int> &taken, const size_t side,
           size_t idx = 0) {
...
std::tuple<std::string, std::string> p20(const std::string &input) {
    ...
    size_t SIDE = 0;
    while(SIDE*SIDE < tiles.size()) SIDE++;
    ...
    auto flipimg = [](const std::vector<std::string> &img) {
    ...
    for (int k = 0; k < 8; k++) {
        ...
        if (k >= 4) bigpicture = flipimg(bigpicture);
        ...
        for (size_t y = 0; y < bigpicture.size(); y++) {
            for (size_t x = 0; x < bigpicture[y].size(); x++) {
                ...
                for (auto[dy, dx] : points) {
                    if (y + dy >= bigpicture.size() || x + dx >= bigpicture[y + dy].size() ||
                        bigpicture[y + dy][x + dx] != '#') {
                        ok = false;
                        break;
                    }
                }
