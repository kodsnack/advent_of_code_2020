//  Watch live coding of this solution at Youtube https://youtu.be/tjbA_02rKKc
//  main.cpp
//  AdventOfCode201212_2
//
...
int main(int argc, const char * argv[]) {
     ...
     */
    NavigationInstructions navigation_instructions = from_string_literal(pData); // Part 1,2

    // Start
    Vector const origo {0,0};
    Vector const east {1,0};
    Vector boat_initial_position {origo};
    Boat boat{origo,east,{10,-1}}; // waypoint starts 10 units east and 1 unit north
    ...
    boat = std::accumulate(navigation_instructions.begin(), navigation_instructions.end(), boat,[](auto const& boat,auto const instruction){
        ...
        switch (instruction.command) {
            // We need to MOVE THE WAYPOINT :)
            //Action N means to move north by the given value.
            case 'N': {
                std::cout << "Move Waypoint North " << instruction.argument;
                result = Boat {boat.position,boat.orientation,boat.waypoint + north*instruction.argument};
            } break;
            //Action S means to move south by the given value.
            case 'S': {
                std::cout << "Move Waypoint South " << instruction.argument;
                result = Boat {boat.position,boat.orientation,boat.waypoint + south*instruction.argument};
            } break;
            //Action E means to move east by the given value.
            case 'E': {
                std::cout << "Move Waypoint East " << instruction.argument;
                result = Boat {boat.position,boat.orientation,boat.waypoint + east*instruction.argument};
            } break;
            //Action W means to move west by the given value.
            case 'W': {
                std::cout << "Move Waypoint West " << instruction.argument;
                result = Boat {boat.position,boat.orientation,boat.waypoint + west*instruction.argument};
            } break;
            //Action L means to turn left the given number of degrees.
            case 'L': {
                std::cout << "Rotate waypoint Left " << instruction.argument << " around boat";
                result = Boat {boat.position,boat.orientation,vector_rotate(boat.waypoint, -instruction.argument)};
            } break;
            //Action R means to turn right the given number of degrees.
            case 'R': {
                std::cout << "Rotate Waypoint Right " << instruction.argument << " around boat";
                result = Boat {boat.position,boat.orientation,vector_rotate(boat.waypoint, instruction.argument)};
            } break;
            //Action F means to add the waypoint vector multiplied by the argument to our position!
            case 'F': {
                std::cout << "Translate Forward " << instruction.argument;
                result = Boat {boat.position + boat.waypoint*instruction.argument, boat.orientation,boat.waypoint};
            } break;
        }
        // Log
        std::cout << "\n"
        ...
        << " waypoint at {" << result.waypoint.x << "," << result.waypoint.y << "}";
        return result;
    });
