//
//  main.cpp
//  CodeOfAdvent201202_1
//
//  Created by Kjell-Olov Högdal on 2020-12-02.
//
...
const char* pData = R"(6-8 s: svsssszslpsp
3-4 n: gncn
...
int main(int argc, const char * argv[]) {
    ...
    std::vector<Entry> v;
    while (data_stream >> entry.low >> entry.high >> entry.sChar >> entry.sPassword) {
        v.push_back(entry);
    }
    auto passed_count = std::count_if(
         v.begin()
        ,v.end()
        ,[](const Entry& entry) {
            auto char_count = std::count(entry.sPassword.begin(), entry.sPassword.end(), entry.sChar[0]);
            bool result = ((entry.low <= char_count) and (char_count <= -entry.high));
            if (result) std::cout << "\nPassed: " << entry.low << entry.high << " " << entry.sChar << " " << entry.sPassword << " " << entry.sChar[0] << "_count " << char_count;
            else std::cout << "\nFailed: " << entry.low << entry.high << " " << entry.sChar << " " << entry.sPassword << " " << entry.sChar[0] << "_count " << char_count;;
            return result;
        }
    );
    std::cout << "\n\n ==> Passed = " << passed_count;
