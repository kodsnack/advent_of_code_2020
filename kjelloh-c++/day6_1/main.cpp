//  See youtube https://youtu.be/CT7mlXZkltc
//  main.cpp
//  CodeOfAdvent201206_1
//
...
struct CustomsEntrySetter {
    CustomsEntry& m_customs_entry;
    void operator()(char ch) {
        m_customs_entry.insert(ch);
    }
};
...
int main(int argc, const char * argv[]) {
    ...
    CustomsEntrySetter customs_entry_setter {customs_entry};
    std::vector<CustomsEntry> customs_entries {};
    while (std::getline(in,sEntry)) {
        if (sEntry.size() == 0) {
            // New Group
            customs_entries.push_back(customs_entry_setter.m_customs_entry);
            customs_entry_setter.m_customs_entry.clear();
        }
        else {
            // note the yes answers in this entry
            std::for_each(sEntry.begin(), sEntry.end(), customs_entry_setter);
        }
    }
    if (customs_entry_setter.m_customs_entry.size() > 0) {
        // last group
        customs_entries.push_back(customs_entry_setter.m_customs_entry);
    }
    // Log yes entries
    std::cout << "\n<Grouped Yes Answers>";
    for (auto const& yes_set : customs_entries) {
        std::cout << "\nYes Set:";
        for (char ch : yes_set) {
            std::cout << '[' << ch << ']';
        }
        std::cout << " yes_count = " << yes_set.size();
    }
    // transform the customs "yes" answers to vector of counts
    std::vector<unsigned int> yes_counts {};
    std::transform(customs_entries.begin(), customs_entries.end(), std::back_inserter(yes_counts),[](auto const& entry){
        return entry.size();
    });
    // Log yes counts vector
    std::cout << "\n\n<Yes Counts Vector>";
    for (auto const& yes_count : yes_counts) {
        std::cout << "\n  yes_count = " << yes_count;
    }
    int counts_sum = std::accumulate(yes_counts.begin(), yes_counts.end(), 0);
    std::cout << "\nSum of Counts = " << counts_sum;
    std::cout << "\n\n";
    return 0;
}
