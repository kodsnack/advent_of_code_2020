//  See youtube https://youtu.be/Uh6GTkQ3_C8
//  main.cpp
//  AdventOfCode201207_1
//
//  Created by Kjell-Olov Högdal on 2020-12-07.
//

// WARNING! This code does the work - but is not tidy.
// I failed to realise the parent bag search would create duplicates!
// So the final result is a quick fix where I assemble the complete parents list (global parent_bag_ids) and trasnform it into a set to get rid of duplicates.
// And the old approach of recursivelly count the parents is still somewhat present.
// I am sorry for that...
...
using BagId = std::vector<std::string>;
using BagRule = std::vector<BagId>;
using BagRules = std::vector<BagRule>;

void log_id(BagId const& id) {
    for (auto const& sToken : id) {
        std::cout << ' ' << sToken;
    }
}

void log_rule(BagRule const& rule) {
    for (auto const& id : rule) {
        std::cout << '[';
        log_id(id);
        std::cout << ']';
    }
...
bool rule_allows_bag(BagRule const& rule,BagId const& bag_id) {
    return std::any_of(rule.begin()+1, rule.end(), [&bag_id](auto const& id) {
        return std::all_of(bag_id.begin(), bag_id.end(), [&id](auto const& sToken){
            return std::any_of(id.begin(), id.end(), [&sToken](auto const& sIdToken){
                bool result = (sToken == sIdToken);
                // std::cout << " " << sToken << " == " << sIdToken << " " << result;
                return result;
            });
        });
    });
...
using BagIds = std::vector<BagId>;
BagIds get_parents_of_bag_id(BagId const& bag_id,BagRules const& rules);
unsigned long parent_count_of_child(BagId const& bag_id,BagRules const& rules);
unsigned long parent_count_of_childs(BagIds const& ids,BagRules const& rules);

BagIds parent_bag_ids {};

BagIds get_parents_of_bag_id(BagId const& bag_id,BagRules const& rules) {
    BagIds result {};
    BagRules parent_rules {};
    std::copy_if(rules.begin(), rules.end(), std::back_inserter(parent_rules), [&bag_id](auto const& rule){
        return rule_allows_bag(rule,bag_id);
    });
    std::transform(parent_rules.begin(), parent_rules.end(), std::back_inserter(result), [](auto const& rule){
        return rule[0];
    });
    std::cout << "\nget_parents_of_bag_id(";
    log_id(bag_id);
    std::cout << ")";
    ...
    std::cout << "\n\tCount = " << result.size();
    parent_bag_ids.insert(std::begin(parent_bag_ids), result.begin(), result.end());
    return result;
}

unsigned long parent_count_of_child(BagId const& bag_id,BagRules const& rules) {
    auto parents = get_parents_of_bag_id(bag_id,rules);
    unsigned long result = parents.size() + parent_count_of_childs(parents,rules);
    std::cout << "\nparent_count_of_child(";
    log_id(bag_id);
    std::cout << ") = " << result;
    return result;
}
...
unsigned long parent_count_of_childs(BagIds const& ids,BagRules const& rules) {
    unsigned long result = std::accumulate(ids.begin(), ids.end(), 0,[&rules](auto const& sum,auto const& id) {
        return sum + parent_count_of_child(id,rules);
    });
    std::cout << "\nparent_count_of_childs";
...
int main(int argc, const char * argv[]) {
    ...
    BagRules bag_rules {};
    std::string sEntry;
    // Split in data into entries
    while (std::getline(in,sEntry)) {
        std::cout << "\nRule Scentences:";
        std::string sSentence;
        std::basic_istringstream<char> entry {sEntry};
        // Split Sentences on ','
        BagRule bag_rule {};
        while (std::getline(entry, sSentence,',')) {
            std::cout << "[" << sSentence << "]";
            // split tokens on space
            std::string sToken {};
            std::basic_istringstream<char> sentence {sSentence};
            BagId bag_id {};
            while (sentence >> sToken) {
                if (sToken == "contain") {
                    // Split to next BagId
                    bag_rule.push_back(bag_id);
                    bag_id.clear();
                }
                else if (sToken.find("bag") != std::string::npos) {
                    // Skip
                }
                else {
                    // put into bag_id
                    bag_id.push_back(sToken);
                }
            }
            bag_rule.push_back(bag_id);
        }
        bag_rules.push_back(bag_rule);
    }
    // Log the parsed bag rules
    for (auto const& rule : bag_rules ) {
        std::cout << "\nRule<";
        log_rule(rule);
        std::cout << ">";
    }
    // OK, lets count the number of ways to carry a "shiny gold" bag?
    std::cout << "\n\n";
    BagId const shiny_gold_id {"shiny","gold"};
    auto shiny_gold_parent_bags_count = parent_count_of_child(shiny_gold_id,bag_rules);
    
    std::sort(parent_bag_ids.begin(), parent_bag_ids.end());
    for (auto const& id : parent_bag_ids) {
        std::cout << "\n";
        log_id(id);
    }
    
    std::cout << "\n\nFinal Set of Carrying Bags";
    std::set<BagId> parents_id_set(parent_bag_ids.begin(),parent_bag_ids.end());
    for (auto const& id : parents_id_set) {
        std::cout << "\n";
        log_id(id);
    }
    std::cout << "\n\nCarrying Bags Count = " << parents_id_set.size();

    
    /*
    
    BagRules shiny_gold_bags {};
    auto shiny_gold_bags_iter = std::copy_if(bag_rules.begin(), bag_rules.end(),std::back_inserter(shiny_gold_bags) , [&shiny_gold_id](auto const& rule) {
        return rule_allows_bag(rule, shiny_gold_id);
    });
    
    // One level up
    std::vector<BagId> ids_of_bags_with_shiny_gold;
    for (auto const& rule : shiny_gold_bags) {
        ids_of_bags_with_shiny_gold.push_back(rule[0]);
    }
    ...
    /*
    auto direct_carry_count = std::count_if(bag_rules.begin(), bag_rules.end(), [&shiny_gold_id](auto const& rule){
        bool result = rule_allows_bag(rule, shiny_gold_id);
        if (result) {
            std::cout << "\nCan Carry ";
            log_id(shiny_gold_id);
            std::cout << " Rule:";
            log_rule(rule);
        }
        return result;
    ...
    std::cout << "\n\nBag Count = " << shiny_gold_bags.size();
     */
...
char const* pData = R"(dull silver bags contain 2 striped magenta bags, 2 dark coral bags, 1 bright orange bag, 4 plaid blue bags.
