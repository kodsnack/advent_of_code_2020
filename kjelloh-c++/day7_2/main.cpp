...
BagRules from_stream(std::istream& in) {
    BagRules bag_rules {};
    std::string sEntry;
...
struct Graph {
    ...
    Vertices vertices;
    Edges edges;
    Vertex vertex(Name const& name) const {
        auto iter = std::find(names.begin(), names.end(), name);
        if (iter != names.end()) {
            return static_cast<Vertex>((iter - names.begin()));
        }
        else {
            return static_cast<Vertex>(names.size()); // invalid index (not found)
        }
    }
    Name name(Vertex const& vertex) const {
        Name result {"NULL"};
        if (vertex < names.size()) {
            result = names[vertex];
        }
        return result;
    }
...
using Contains = std::pair<Count,std::string>;
Contains to_contains(std::vector<std::string> const& ss) {
    Contains result {1,""};
    auto iter = ss.begin();
    if (std::all_of(ss[0].begin(), ss[0].end(), [](char c){
        return std::isdigit(c) > 0;
    })) {
        // Must be a better way of doing this??
        result.first = std::stoi(ss[0]);
        ++iter;
    }
    std::string sId {*iter};
    while (++iter != ss.end()) {
...
Graph rules_to_graph(BagRules const& bag_rules) {
    Graph result;
    // assemble names and vertices
    for (auto const& entry : bag_rules) {
        auto contains {to_contains(entry[0])};
        Name name {contains.second};
        result.names.push_back(name);
        std::cout << "\n" << name << " -> " << result.names.size()-1;
        result.vertices.push_back(static_cast<Vertex>(result.names.size()-1)); // Asume one rule entry per bag
    }
    // Assemble weighted edges ("contains" relationsships)
    for (auto const& rules_entry : bag_rules) {
        auto iter = rules_entry.begin();
        auto first_contains {to_contains(*iter)};
        Name from_name {first_contains.second};
        Vertex from_vertex = result.vertex(from_name);
        while (++iter != rules_entry.end()) {
            auto end_contains {to_contains(*iter)};
            Vertex end_vertex {result.vertex(end_contains.second)};
            Edge edge {{from_vertex,end_vertex},end_contains.first};
...
void print_graph(Graph const& graph) {
    std::cout << "\nGraph vertex_count:" << graph.vertices.size() << " edge_count:" << graph.edges.size();
    std::cout << "\n\t<names>";
    ...
    for (int i = 0; i < graph.edges.size();++i) {
        std::cout << "\n\t" << graph.edges[i].vs.first << " -- contains " << graph.edges[i].count << " of " << graph.edges[i].vs.second;
    }
}

Count sub_graph_vertex_count(Vertex const& vertex, Graph const& graph) {
    Count result {0};
    std::cout << "\nsub_graph_vertex_count vertex:" << vertex << " i.e., " << graph.name(vertex);
    if (vertex < graph.vertices.size()) {
        result += 1; // count this vertex
        Edges contains {};
        std::copy_if(graph.edges.begin() , graph.edges.end(), std::back_inserter(contains), [&vertex](auto const& edge){
            return (edge.vs.first == vertex); // from
        });
        // Log edges to childs
        for (auto const& edge : contains) {
            std::cout << "\n\tedge " << edge.vs.first << " --> " << edge.vs.second;
        }
        // Count child nodes
        for (auto const& edge : contains) {
            result += edge.count * sub_graph_vertex_count(edge.vs.second, graph);
        }
    }
    return result;
}
...
int main(int argc, const char * argv[]) {
    ...
    BagRules bag_rules {from_stream(in)};
    ...
    // Build a graph with weighted edegs
    auto graph = rules_to_graph(bag_rules);
    print_graph(graph);

    auto result = sub_graph_vertex_count(graph.vertex("shinygold"),graph)-1; // don't count top shiny gold bag :)
    std::cout << "\n\nResult = " << result;
...
char const* pExample = R"(light red bags contain 1 bright white bag, 2 muted yellow bags.
...
char const* pExample2 = R"(shiny gold bags contain 2 dark red bags.
