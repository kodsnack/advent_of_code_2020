...
solution :: Room -> Int
solution = (countOccupied . Map.elems) . stable (nextRoom countVisible 5)
...
countVisible :: CountingMethod
countVisible room pos = countOccupied . mapMaybe (scan room pos) $ directions

scan :: Room -> Pos -> Pos -> Maybe Seat
scan room p d =
    case Map.lookup p' room of
      Just Floor -> scan room p' d
      seat -> seat
    where p' = changePos p d
          changePos (x, y) (x', y') = (x + x', y + y')
