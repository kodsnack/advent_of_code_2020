...
public class Day extends com.github.rccookie.adventofcode.util.Day {
    ...
    private Set<Group> parseGroups(Function<Set<Person>, Group> groupCreater) {
        Set<Group> groups = new HashSet<>();
        Set<Person> current = new HashSet<>();
        for(String line : inputInLines()) {
            if(line.length() == 0) {
                groups.add(groupCreater.apply(current));
                current = new HashSet<>();
                continue;
            }
            current.add(new Person(line.toCharArray()));
        }
        if(current.size() > 0) groups.add(groupCreater.apply(current));
        return groups;
    }
}
