package rccookie.year2020.day9;
...
public class Day extends com.github.rccookie.adventofcode.util.Day {
    ...
    public long resultPart1() throws Exception {
        ...
        long[] numbers = new long[26];
        mainLoop:
        for(int i=0; i<input.length; i++) {
            long n = Long.parseLong(input[i]);
            System.arraycopy(numbers, 0, numbers, 1, numbers.length - 1);
            numbers[0] = n;
            if(i < numbers.length) continue mainLoop;

            for(int j=1; j<numbers.length - 1; j++) {
                for(int k=j+1; k<numbers.length; k++) {
                    if(numbers[0] == numbers[j] + numbers[k]) continue mainLoop;
                }
            }
            return numbers[0];
        }
    ...
    /**
     * <h2 id="part2">--- Part Two ---</h2><p>The final step in breaking the XMAS encryption relies on the invalid number you just found: you must <em>find a contiguous set of at least two numbers</em> in your list which sum to the invalid number from step 1.</p>
    ...
    public long resultPart2() throws Exception {
        ...
        long target = resultPart1();
        long[] numbers = inputNumbers();

        for(int start = 0; start < numbers.length; start++) {
            long sum = 0;
            List<Long> summingNumbers = new ArrayList<>();
            int i = start;
            for(;sum < target && i<numbers.length; i++) {
                sum += numbers[i];
                summingNumbers.add(numbers[i]);
            }
            if(sum == target) {
                long[] array = new long[summingNumbers.size()];
                for(int j=0; j<array.length; j++) array[j] = summingNumbers.get(j);
                return Geometry.min(array) + Geometry.max(array);
            }
        }
